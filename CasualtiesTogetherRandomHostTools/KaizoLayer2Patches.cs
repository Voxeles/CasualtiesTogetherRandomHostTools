    ...
    private static class RemoveOilPipesPatch
    {
        ...
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            codeMatcher.MatchForward(false, 
                ...
                .ThrowIfInvalid($"{nameof(RemoveOilPipesPatch)}.{nameof(Transpiler)} could not find a match (oilpipe ldstr)!")
                .MatchBack(true, 
                    new CodeMatch(OpCodes.Ldfld, AccessTools.Field(typeof(WorldGeneration), nameof(WorldGeneration.chunkWidth))))
                .ThrowIfInvalid($"{nameof(RemoveOilPipesPatch)}.{nameof(Transpiler)} could not find a match (chunkWidth ldfld)!")
                .Advance(1)
                .Insert(
